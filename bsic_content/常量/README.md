...
const int i,j = 0;//error i must be initalized
```
上面有两个错误：
* b 为常量不能修改
* i 为常量 必须进行初始化（因为常量在定义以后就不能进行修改）
...
cosnt char * const a；//指向const对象的const指针。
```
>小结
...
可以修改const指针所指向的值的，但是不能通过const对象指针来进行而已！如下修改：
```c++
int *ptr1 = &val;
*ptr1=4;
cout<<*ptr<<endl;
